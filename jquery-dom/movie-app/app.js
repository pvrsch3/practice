//When the form is submitted, capture the values for each 
//of the inputs and append them to the DOM along with a 
//button to remove each title and rating from the DOM.
// id to keep track of which element to remove (this would 
//be better not in global scope)
let currentId = 0;
...
$(function() {
  ...
  // when the delete button is clicked, remove the closest
  // parent tr and remove from the array of movies
  ...
  $("tbody").on("click", ".btn.btn-danger", function(evt) {
    ...
    let indexToRemoveAt = moviesList.findIndex(movie => 
        movie.currentId === +$(evt.target).data("deleteId"))
...
//Further study!!!
  ...
  //th scope title and rating class .fas
